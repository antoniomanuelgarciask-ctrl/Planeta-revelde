...
async function saveStoredItems(items) {
    // Ahora los datos viven en la nube
...
// 1. Crea la imagen o video de la tarjeta
function createMediaElement(item) {
    if (!item.mediaUrl) return null;

    const isVideo = /\.(mp4|webm|ogg|mov)(\?.*)?$/i.test(item.mediaUrl) || item.mediaType === 'video';
    
    if (isVideo) {
        const video = document.createElement("video");
        video.controls = true;
        video.src = item.mediaUrl;
        video.className = "tarjeta-media";
        return video;
    } else {
        const img = document.createElement("img");
        img.src = item.mediaUrl;
        img.className = "tarjeta-media";
        img.alt = item.title;
        return img;
    }
}

// 2. Crea la tarjeta visual (Noticias, Inicio, etc.)
function createPublicacionCard(item) {
    ...
    card.innerHTML = `
        <div class="tarjeta-cuerpo">
            <span class="etiqueta">${item.section}</span>
            <h3>${item.title || 'Sin título'}</h3>
            <p>${item.description || ''}</p>
            ${item.linkUrl ? `<a href="${item.linkUrl}" target="_blank" class="boton-accion">Saber más</a>` : ""}
        </div>
    `;

    if (mediaElement) {
        card.prepend(mediaElement);
    }

    return card;
}

// 3. Renderiza la lista de gestión en el Panel Admin
async function renderAdminItems(container) {
    if (!container) return;
    container.innerHTML = "<p>Cargando lista de gestión...</p>";

    const items = await getStoredItems(); // Ahora viene de Supabase

    container.innerHTML = "";

    if (!items.length) {
        container.innerHTML = "<div class='vacio'><strong>No hay contenido en la nube.</strong></div>";
        return;
    }
    ...
    items.forEach((item) => {
        ...
        card.innerHTML = `
            ...
            <div class="meta">${item.section} | ${item.created_at ? new Date(item.created_at).toLocaleDateString() : 'Reciente'}</div>
            <p>${item.description || ''}</p>
            <div class="fila-botones">
                <button type="button" class="boton-borrar" data-id="${item.id}">Eliminar de la nube</button>
            </div>
        `;
        
        // Evento para borrar de Supabase
        card.querySelector('.boton-borrar').onclick = async () => {
            if (confirm("¿Seguro que quieres borrar esto de la nube?")) {
                const { error } = await supabaseClient.from('publicaciones').delete().eq('id', item.id);
...
// Funciones de utilidad que pedía tu código
