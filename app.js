...
function getDailyImageHelpText() {
    if (hasDailyImageManifestConfig()) {
        return "La imagen diaria puede salir de la biblioteca del panel admin o del JSON externo configurado en app.js. Si una fecha no coincide, la web rota automaticamente entre las imagenes sin fecha fija.";
    }

    return "La imagen diaria usa primero la biblioteca del panel admin. Si algun dia quereis leerla desde otra web, tambien podeis rellenar DAILY_IMAGE_MANIFEST_URL en app.js con un JSON de URLs o { images: [...] }.";
}
...
function buildBugReportModal() {
    ...
    modal.innerHTML = `
        <div class="onboarding-card bug-report-card">
            ...
            <p>Rellena el formulario y enviaremos el aviso a ${BUG_REPORT_EMAIL} sin sacarte de la web.</p>

            <div id="bug-report-status" class="estado ok">Describe el error y, si puedes, adjunta capturas de pantalla.</div>
...
async function submitBugReport(form) {
    ...
    try {
        ...
        showStatus(status, "ok", "Bug enviado correctamente. Gracias por avisar.");
        form.reset();
        renderBugReportPreview(null);
